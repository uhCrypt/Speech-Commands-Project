...
if platform_Version == "32bit" or platform_Version == "64bit":
    ...
    if int(python_Version_Digit) >= 34 and int(python_Version_Digit) <= 310:
        ...
        if 'keyboard' in installed_packages:
            import pyautogui as pyg
            if pyg.__version__ == "0.9.53":
                print("Keyboard is already installed, skipping step.\n")
            else:
                userInput = input("Keyboard is installed but the wrong version is installed. Would you like to install the recommended version?\n\nY/N\n")
                if userInput.lower() == "y":
                    installModule("Keyboard", "keyboard==0.13.5")
                else:
                    print("Skipping Keyboard installation of recommended version.\n")
        else:
            installModule("Keyboard", "keyboard==0.13.5")
